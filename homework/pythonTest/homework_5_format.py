...
for line in lines:
    ...
    # 规划一个存放的地方，我也想到了要放到alist,但是apend方法一次只能加一个元素，没有想到列表套列表
    # 使用循环判断的方式创建子列表[类型，大小]
    # 1.如果这行文件的类型，不在结果的列表里---新增一个列表
    # 2.如果这行文件的类型，在结果的列表里---累加到这个列表
    ...
    # 新增的条件要先判断，有没有这个类型
    if inFlage == False:
        alist.append([fileType,fileSize])
